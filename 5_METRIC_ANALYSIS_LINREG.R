...
# Kennzahlen fuer die multiple Regression bereit. Dazu gehoeren:
...
# - 3 Dokumente (fuer Fall A-C) - Guete von Punktschaetzern:       Mean, Bias, RMSE, Median, IQR und Range
...
# DATEN LADEN ----------------------------------------------------------------
# ============================================================================ #
load("4.1_RESULTS_SIMSTUD_LINREG.RData")    # simulierte Daten laden
metric_data_linreg <- final.results.linreg  # Daten in das Objekt 'metric_data_linreg' speichern

# Ueberpruefen, ob fehlende Werte in den geladenen Daten vorhanden sind. 
any(is.na(metric_data_linreg))
...
for (case_id in case_ids) {
  for (ref_id in ref_ids) {
    for (n_total in n_total_vals) {
      ...
      filtered_data <- subset(metric_data_linreg, CASE.ID == case_id & REF.ID == ref_id & n.total == n_total)
      
      # Berechnung der Mittelwerte der geschaetzten Achsenabschnitt- und Effektstaerken
      mean_est_int <- format_value(mean(filtered_data$EST.INT))
      mean_est_slope_one <- format_value(mean(filtered_data$EST.SLOPE.ONE))
      mean_est_slope_two <- format_value(mean(filtered_data$EST.SLOPE.TWO))
      
      # Berechnung von Bias fuer Achsenabschnitt und Effektstaerken
      bias_int <- bias(filtered_data$TRUE.INT, filtered_data$EST.INT)
      bias_slope_one <- bias(filtered_data$TRUE.SLOPE.ONE, filtered_data$EST.SLOPE.ONE)
      bias_slope_two <- bias(filtered_data$TRUE.SLOPE.TWO, filtered_data$EST.SLOPE.TWO)
      
      # Berechnung von RMSE fuer Achsenabschnitt und Effektstaerken
      rmse_int <- rmse(filtered_data$TRUE.INT, filtered_data$EST.INT)
      rmse_slope_one <- rmse(filtered_data$TRUE.SLOPE.ONE, filtered_data$EST.SLOPE.ONE)
      rmse_slope_two <- rmse(filtered_data$TRUE.SLOPE.TWO, filtered_data$EST.SLOPE.TWO)
      
      # Berechnung von Median und IQR fuer Achsenabschnitt und Effektstaerken
      stats_int <- calc_stats(filtered_data$EST.INT)
      stats_slope_one <- calc_stats(filtered_data$EST.SLOPE.ONE)
      stats_slope_two <- calc_stats(filtered_data$EST.SLOPE.TWO)
      
      # Coverage Probability fuer Achsenabschnitt und Effektstaerken mit herkoemmlichen Konfidenzintervallen
      coverage_prob_int_norm <- coverage_prob(filtered_data$CI.INT)
      coverage_prob_slope_one_norm <- coverage_prob(filtered_data$CI.SLOPE.ONE)
      coverage_prob_slope_two_norm <- coverage_prob(filtered_data$CI.SLOPE.TWO)
      
      # Coverage Probability fuer Achsenabschnitt und Effektstaerken mit Bootstrap-Konfidenzintervallen
      coverage_prob_int_boot <- coverage_prob(filtered_data$BOOT.CI.INT)
      coverage_prob_slope_one_boot <- coverage_prob(filtered_data$BOOT.CI.SLOPE.ONE)
      coverage_prob_slope_two_boot <- coverage_prob(filtered_data$BOOT.CI.SLOPE.TWO)
      
      # Hit Rate und False Alarm Rate fuer Slope One mit herkoemmlichen Konfidenzintervallen
      hit_rate_slope_one_norm <- hit_rate_effect(filtered_data$TRUE.SLOPE.ONE, filtered_data$CI.SLOPE.ONE.LOWER, filtered_data$CI.SLOPE.ONE.UPPER)
      false_alarm_rate_slope_one_norm <- false_alarm_rate_effect(filtered_data$TRUE.SLOPE.ONE, filtered_data$CI.SLOPE.ONE.LOWER, filtered_data$CI.SLOPE.ONE.UPPER)
      
      # Hit Rate und False Alarm Rate fuer Slope Two mit herkoemmlichen Konfidenzintervallen
      hit_rate_slope_two_norm <- hit_rate_effect(filtered_data$TRUE.SLOPE.TWO, filtered_data$CI.SLOPE.TWO.LOWER, filtered_data$CI.SLOPE.TWO.UPPER)
      false_alarm_rate_slope_two_norm <- false_alarm_rate_effect(filtered_data$TRUE.SLOPE.TWO, filtered_data$CI.SLOPE.TWO.LOWER, filtered_data$CI.SLOPE.TWO.UPPER)
      
      # Hit Rate und False Alarm Rate fuer Slope One mit Bootstrap-Konfidenzintervallen
      hit_rate_slope_one_boot <- hit_rate_effect(filtered_data$TRUE.SLOPE.ONE, filtered_data$BOOT.CI.SLOPE.ONE.LOWER, filtered_data$BOOT.CI.SLOPE.ONE.UPPER)
      false_alarm_rate_slope_one_boot <- false_alarm_rate_effect(filtered_data$TRUE.SLOPE.ONE, filtered_data$BOOT.CI.SLOPE.ONE.LOWER, filtered_data$BOOT.CI.SLOPE.ONE.UPPER)
      
      # Hit Rate und False Alarm Rate fuer Slope Two mit Bootstrap-Konfidenzintervallen
      hit_rate_slope_two_boot <- hit_rate_effect(filtered_data$TRUE.SLOPE.TWO, filtered_data$BOOT.CI.SLOPE.TWO.LOWER, filtered_data$BOOT.CI.SLOPE.TWO.UPPER)
      false_alarm_rate_slope_two_boot <- false_alarm_rate_effect(filtered_data$TRUE.SLOPE.TWO, filtered_data$BOOT.CI.SLOPE.TWO.LOWER, filtered_data$BOOT.CI.SLOPE.TWO.UPPER)
      
      # Speichern der Ergebnisse fuer die aktuelle Kombination in einer Liste
      results_list[[paste(case_id, ref_id, n_total, sep = "_")]] <- list(
        ...
        CP_INT_NORM = coverage_prob_int_norm, CP_SlopeOne_NORM = coverage_prob_slope_one_norm, CP_SlopeTwo_NORM = coverage_prob_slope_two_norm,
        CP_INT_BOOT = coverage_prob_int_boot, CP_SlopeOne_BOOT = coverage_prob_slope_one_boot, CP_SlopeTwo_BOOT = coverage_prob_slope_two_boot,
        
        # Guete von Variablenselektion
        HR_SlopeOne_NORM = hit_rate_slope_one_norm, FAR_SlopeOne_NORM = false_alarm_rate_slope_one_norm,
        HR_SlopeTwo_NORM = hit_rate_slope_two_norm, FAR_SlopeTwo_NORM = false_alarm_rate_slope_two_norm,
        HR_SlopeOne_BOOT = hit_rate_slope_one_boot, FAR_SlopeOne_BOOT = false_alarm_rate_slope_one_boot,
        HR_SlopeTwo_BOOT = hit_rate_slope_two_boot, FAR_SlopeTwo_BOOT = false_alarm_rate_slope_two_boot
      )
    }
  }
}
...
linreg.analysis.results <- do.call(rbind, lapply(results_list, as.data.frame))

View(linreg.analysis.results)

# Speichern der Ergebnisse in einer RData-Datei
save(linreg.analysis.results, file = "5.1_METRIC_ANALYSIS_LINREG.RData")
...
for (case_id in case_ids) {
  ...
  # Guete von Konfidenzintervallen
  cp_df <- case_data[, c("Case", "Reference", "n", 
                         "CP_INT_NORM", "CP_SlopeOne_NORM", "CP_SlopeTwo_NORM", 
                         "CP_INT_BOOT", "CP_SlopeOne_BOOT", "CP_SlopeTwo_BOOT")]
  
  # Guete von Variablenselektion
  hr_far_df <- case_data[, c("Case", "Reference", "n", 
                             "HR_SlopeOne_NORM", "FAR_SlopeOne_NORM", 
                             "HR_SlopeTwo_NORM", "FAR_SlopeTwo_NORM", 
                             "HR_SlopeOne_BOOT", "FAR_SlopeOne_BOOT", 
                             "HR_SlopeTwo_BOOT", "FAR_SlopeTwo_BOOT")]
  
  # Export in drei separate Word-Dokumente
  
  # Dokument fuer Mean, Bias, RMSE, Median, IQR, Range
  doc1 <- read_docx()
  doc1 <- body_add_flextable(doc1, flextable(mean_bias_rmse_median_iqr_range_df))
  print(doc1, target = file.path(output_dir, 
                                 paste0("Case_", case_id, "_A_Mean_Bias_RMSE_Median_IQR_Range_LinReg.docx")))
  
  # Dokument fuer Coverage Probabilities
  doc2 <- read_docx()
  doc2 <- body_add_flextable(doc2, flextable(cp_df))
  print(doc2, target = file.path(output_dir, 
                                 paste0("Case_", case_id, "_B_Coverage_Probabilities_LinReg.docx")))
  
  # Dokument fuer Hit Rate und False Alarm Rate
  doc3 <- read_docx()
  doc3 <- body_add_flextable(doc3, flextable(hr_far_df))
  print(doc3, target = file.path(output_dir, 
                                 paste0("Case_", case_id, "_C_HitRate_FalseAlarmRate_LinReg.docx")))
}
